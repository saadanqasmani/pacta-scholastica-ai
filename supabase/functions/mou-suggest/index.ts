...
const LANGUAGE_INSTRUCTIONS: Record<string, string> = {
  en: "Generate all clause titles and content in English.",
  tr: "Tüm madde başlıklarını ve içeriklerini Türkçe olarak oluştur.",
  de: "Erstelle alle Klauseltitel und Inhalte auf Deutsch.",
  ar: "أنشئ جميع عناوين ومحتويات البنود باللغة العربية.",
};
...
serve(async (req) => {
  ...
  try {
    ...
    const { cooperation_scope, initiator_name, partner_name, existing_clauses, language } = await req.json();
...
Please suggest 3-5 professional MOU clauses that cover the essential aspects of this partnership. Each clause should be specific, actionable, and balanced.`;
                  ...
                  clauses: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        category: { type: 'string', enum: ['governance', 'academic', 'financial', 'mobility', 'research', 'termination', 'general'] },
    ...
    return new Response(JSON.stringify({ clauses: [] }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error: unknown) {
