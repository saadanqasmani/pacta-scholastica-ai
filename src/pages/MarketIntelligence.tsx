...
import {
  TrendingUp, TrendingDown, AlertTriangle, CheckCircle, PauseCircle, XCircle,
  Globe, Users, Target, BarChart3, RefreshCw, Sparkles,
} from 'lucide-react';
...
export default function MarketIntelligence() {
  ...
  const generateAnalysis = async () => {
    ...
    try {
      ...
      toast({ title: t('intelligence.analysisComplete'), description: t('intelligence.analysisCompleteDesc') });
    } catch (err) {
      const message = err instanceof Error ? err.message : t('intelligence.analysisFailed');
      toast({ title: t('intelligence.analysisError'), description: message, variant: 'destructive' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (selectedUniversity?.id) generateAnalysis();
  ...
  if (!selectedUniversity) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-muted-foreground">{t('intelligence.selectUniversity')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t('intelligence.title')}</h1>
          <p className="text-muted-foreground">
            {t('intelligence.recruitmentMarketAnalysis')} {selectedUniversity.name}
          </p>
        </div>
        <Button onClick={generateAnalysis} disabled={isLoading}>
          {isLoading ? (
            <><RefreshCw className="mr-2 h-4 w-4 animate-spin" />{t('recruitment.analyzing')}</>
          ) : (
            <><Sparkles className="mr-2 h-4 w-4" />{t('intelligence.refreshAnalysis')}</>
          )}
        </Button>
      </div>
      ...
      {analysis && (
        <>
          <div className="grid gap-4 md:grid-cols-5">
            <Card>
              <CardHeader className="pb-2">
                <CardDescription>{t('intelligence.totalApplications')}</CardDescription>
                <CardTitle className="text-2xl">{analysis.summary.totalApplications.toLocaleString()}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader className="pb-2">
                <CardDescription>{t('intelligence.avgConversion')}</CardDescription>
            ...
            <Card className="border-green-500/20">
              <CardHeader className="pb-2">
                <CardDescription className="flex items-center gap-1">
                  <TrendingUp className="h-3 w-3 text-green-600" />{t('intelligence.marketsToScale')}
                </CardDescription>
            ...
            <Card className="border-yellow-500/20">
              <CardHeader className="pb-2">
                <CardDescription className="flex items-center gap-1">
                  <PauseCircle className="h-3 w-3 text-yellow-600" />{t('intelligence.marketsToPause')}
                </CardDescription>
            ...
            <Card className="border-red-500/20">
              <CardHeader className="pb-2">
                <CardDescription className="flex items-center gap-1">
                  <XCircle className="h-3 w-3 text-red-600" />{t('intelligence.marketsToExit')}
                </CardDescription>
          ...
          <Tabs defaultValue="recommendations" className="space-y-4">
            <TabsList>
              <TabsTrigger value="recommendations">{t('intelligence.recommendations')}</TabsTrigger>
              <TabsTrigger value="metrics">{t('intelligence.metrics')}</TabsTrigger>
            </TabsList>
                ...
                {analysis.recommendations.map((rec) => (
                  <Card key={rec.country} className="relative overflow-hidden">
                    <div className={`absolute top-0 left-0 w-1 h-full ${
                      rec.action === 'scale' ? 'bg-green-500' : rec.action === 'pause' ? 'bg-yellow-500' : 'bg-red-500'
                    }`} />
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="pt-3 border-t space-y-2">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('intelligence.conversionEfficiency')}</span>
                          <span className="font-medium">{rec.keyMetrics.conversionEfficiency}%</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('intelligence.recruitmentQuality')}</span>
                          <span className="font-medium">{rec.keyMetrics.recruitmentQuality}%</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t('intelligence.capacityAlignment')}</span>
            ...
            <TabsContent value="metrics" className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <BarChart3 className="h-5 w-5" />{t('intelligence.marketPerformance')}
                  </CardTitle>
                  <CardDescription>{t('intelligence.detailedBreakdown')}</CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b">
                          <th className="text-left py-3 px-2 font-medium">{t('analytics.country')}</th>
                          <th className="text-right py-3 px-2 font-medium">{t('intelligence.applications')}</th>
                          <th className="text-right py-3 px-2 font-medium">{t('intelligence.offers')}</th>
                          <th className="text-right py-3 px-2 font-medium">{t('intelligence.enrollments')}</th>
                          <th className="text-right py-3 px-2 font-medium">{t('intelligence.conversionPct')}</th>
                          <th className="text-right py-3 px-2 font-medium">{t('intelligence.overOffering')}</th>
                          <th className="text-right py-3 px-2 font-medium">{t('intelligence.wasteRatio')}</th>
                          <th className="text-right py-3 px-2 font-medium">{t('intelligence.agentPct')}</th>
                        </tr>
                      </thead>
                      <tbody>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-2">
                      <Users className="h-4 w-4" />{t('intelligence.channelQuality')}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                      ...
                      return (
                        <div key={market.country} className="space-y-1">
                          ...
                          <div className="flex h-2 rounded-full overflow-hidden bg-muted">
                            <div className="bg-blue-500" style={{ width: `${agentPct}%` }} />
                            <div className="bg-green-500" style={{ width: `${100 - agentPct}%` }} />
                          </div>
                        </div>
                      );
                    })}
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-2">
                      <AlertTriangle className="h-4 w-4" />{t('intelligence.highRiskMarkets')}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                      ...
                      {analysis.recommendations
                        ...
                        .map((rec) => (
                          <div key={rec.country} className="flex items-center justify-between p-3 rounded-lg bg-red-500/5 border border-red-500/10">
                            ...
                            <Badge variant="outline" className="bg-red-500/10 text-red-600">{rec.action}</Badge>
                          </div>
                        ))}
                      {analysis.recommendations.filter((r) => r.riskLevel === 'high').length === 0 && (
                        <p className="text-sm text-muted-foreground text-center py-4">{t('intelligence.noHighRisk')}</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
      ...
      {isLoading && !analysis && (
        <div className="flex flex-col items-center justify-center py-12 space-y-4">
          ...
          <p className="text-muted-foreground">{t('intelligence.generatingReport')}</p>
        </div>
      )}
    </div>
  );
